"""tai-agents — the reference agents package for the TAI ecosystem.
...
each module registers its agent through the global ``tai_app`` handle from
``tai_contract.app`` (``@tai_app.agents.agent(name)`` on a subclass of the
