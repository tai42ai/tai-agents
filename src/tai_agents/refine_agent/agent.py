...
The Evaluator drafts an answer; the Critic reviews it; the Evaluator revises
against that feedback. The two alternate until the Critic emits the approval
token (:data:`~tai_agents.refine_agent.prompt.CRITIC_APPROVAL_MESSAGE`) or the
iteration budget is exhausted — in which case the run fails loudly with a
``RuntimeError`` rather than returning an unapproved draft. An empty critic
response is likewise a loud ``RuntimeError``.

Only the FINAL, approved evaluator pass streams. The whole Evaluator↔Critic
negotiation runs silently in the prelude (each turn is a plain ``ainvoke``); once
the Critic approves, the Evaluator is asked for its final answer and THAT pass is
projected as the normalized :class:`~tai_contract.agent.events.StreamEvent`
vocabulary — the decisive, user-facing stage is the one that streams. ``run``
drains that same stream to the final value via :meth:`Agent._drain`, so the loop
has a single implementation.

With a ``response_format`` set, the loop iterations stay text (the critic reviews
prose) and only the final approved answer is forced into the schema — run on a
second, structured evaluator fed the negotiation history explicitly on a fresh
thread, and surfaced as a terminal
:class:`~tai_contract.agent.events.StructuredFinal`.
"""
...
from tai_contract.agent.events import StreamEvent, StructuredFinal
from tai_contract.app import tai_app
from tai_kit.llm.checkpoint.checkpoint_registry import checkpoint_registry
from tai_kit.llm.middleware.context_overflow import context_overflow_middlewares
from tai_kit.llm.models import get_llm_async
from tai_kit.llm.runtime import build_agent_input, build_user_output
from tai_kit.llm.settings import llm_provider_settings, llm_settings
from tai_kit.logging.settings import logging_settings

from tai_agents._internal.config_util import init_langgraph_config
from tai_agents._internal.reject import reject_unhonored, reject_untitled_response_format
from tai_agents._internal.render import render_message
from tai_agents._internal.stream_events import aproject_agent_events
from tai_agents.refine_agent.prompt import (
...
# set). The agent drives an Evaluator↔Critic negotiation through role-named
# parameters (evaluator_/critic_ messages, providers, llm_kwargs, and langgraph
# configs) and resolves tools from ``tool_names``, so these generic parameters are
# rejected loudly rather than silently dropped. ``tool_names``,
# ``checkpoint_provider``, and ``response_format`` (which forces the final approved
# pass into a structured object) are the contract parameters this agent honors, so
...
async def _run_refine_loop(
    ...
    Returns ``(final_agent, final_input, final_config)`` — the graph, its input,
    and its run config for the final approved evaluator pass the caller streams.

    Without a ``response_format`` this is the loop's own evaluator resumed on its
    checkpointed thread with a "Critic Approved." prompt (the negotiation history
    lives in that thread). With a ``response_format`` set, the final pass instead
    runs on a SECOND evaluator built with that schema (``create_agent(...,
    response_format=...)``) on a FRESH thread, fed the negotiation history read
    from the loop thread's checkpoint EXPLICITLY as input — never a checkpoint
    resume of the text-shaped loop thread into the differently-shaped structured
    graph. Only the final answer is forced; the loop iterations stay text so the
    critic exchange is unaffected.

    Raises ``RuntimeError`` when the Critic returns no feedback or when
    ``max_iterations`` is reached without the approval token — an unapproved draft
    is never returned.
    """
    ...
    # read its negotiation history from the checkpoint and feed it as input to a
    # second, structured evaluator on a fresh thread (never a cross-topology resume).
...
@tai_app.agents.agent("refine_agent")
class RefineAgent(Agent):
    ...
    async def run(self, **kwargs: Any) -> Any:
        """Reject the contract parameters the loop has no seat for, then run the
        loop and return the final approved answer by draining :meth:`astream`.

        Both faces run the same honor-or-raise guard naming their own method, so a
        contract parameter with no seat (e.g. ``tools``/``presets``/``thread_id``)
        is rejected identically on each rather than only inside :meth:`astream`. A
        ...
        validated structured object the final approved pass forces, raising loudly
        if the run produced none.
        """
    ...
    async def astream(
        ...
        **kwargs: Any,
        ...
        """Stream the final evaluator pass; the Evaluator↔Critic loop runs
        silently until approval (or raises at ``max_iterations``).

        ``tool_names`` (resolved through the app tool facet), ``checkpoint_provider``,
        and ``response_format`` are the contract parameters this agent honors; every
        other :class:`~tai_contract.agent.Agent.run` parameter has no seat in the
        role-named loop and is rejected loudly (see :data:`_UNHONORED_REASONS`)
        rather than silently dropped. Plain extension kwargs beyond the contract
        names are left untouched.

        With a ``response_format`` set, the loop iterations stay text (the critic
        sees prose) and only the final approved answer is forced into the schema —
        surfaced as a terminal :class:`~tai_contract.agent.events.StructuredFinal`;
        a requested ``response_format`` the final pass produces none for raises
        loudly after the stream drains rather than silently omitting the frame.
        """
        reject_unhonored(
            "refine_agent.astream", kwargs, _UNHONORED_REASONS, collection_params=_UNHONORED_COLLECTION_PARAMS
        )
        reject_untitled_response_format("refine_agent", response_format)
        resolved_tools = await tai_app.tools.get_client_tools(tool_names) if tool_names else []
