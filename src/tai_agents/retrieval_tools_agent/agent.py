...
:mod:`tai_agents.retrieval_tools_agent.graph`). It is useful when the tool set is
...
from tai_contract.agent import Agent, MessageDelta, MessageFinal, StreamEvent, StructuredFinal
...
from tai_kit.llm.embedding import get_embedding_async
from tai_kit.llm.models import get_llm_async
from tai_kit.llm.runtime import build_agent_input, validate_structured_output
from tai_kit.llm.settings import embedding_settings, llm_provider_settings, llm_settings
...
from tai_agents._internal.reject import (
...
from tai_agents.retrieval_tools_agent.prompt import RETRIEVAL_SYSTEM_MESSAGE
...
@tai_app.agents.agent("retrieval_tools_agent")
class RetrievalToolsAgent(Agent):
    ...
    async def _build(
        ...
        *,
        ...
        resolved_tools = await resolve_tools(tai_app.tools, list(tool_names), list(tools), list(presets))
