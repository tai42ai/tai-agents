...
from tai_kit.llm.middleware.context_overflow import areduce_context, context_overflow_middlewares
...
class RetrievalToolsGraph:
    ...
    def __init__(
        ...
        # The langgraph store keys items by a ``tuple[str, ...]`` namespace. The
        # index is namespaced per resolved tool-set (a stable hash of the sorted
        # tool names) so concurrent sessions with different tool-sets never share
        # one global index — foreign tool keys can't crowd a session's
        # ``tools_limit`` and degrade retrieval. Sorting keeps it deterministic:
        # the same tool-set always maps to the same namespace, so the embedding
        ...
        async def node(tool_calls: list[dict], config: RunnableConfig, *, store: BaseStore) -> dict[str, Any]:
            ...
            for tool_call in tool_calls:
                ...
                try:
                    ...
                    # Only genuine TOOL-LOGIC failures are surfaced back to the
                    # model as a ToolMessage (visible and actionable): a tool that
                    # signals an error via ``ToolException`` or malformed call
                    # args (``ValidationError``). INFRASTRUCTURE-class failures —
                    # a vector-store outage, an embedding-endpoint error, a
                    # connection error — are NOT caught here and propagate loudly
                    # to the caller. The except scope is also narrowed to the
                    # invocation call only, so a programming error in the mapping
                    # below propagates instead of being masked.
                    messages.append(
                        ToolMessage(
                            f"Error: {exc}",
