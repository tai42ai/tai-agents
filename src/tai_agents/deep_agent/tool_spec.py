...
The core :class:`tai_agents.deep_agent.spec.ResolvedSubAgentSpec` carries a
``tools: list[StructuredTool]`` field, which is not JSON-serializable and so
cannot cross the tool boundary. :class:`DeepSubAgentSpec` is the JSON authoring
shape exposed to JSON callers: ``tools`` are tool *names* (resolved through
``tai_app.tools.get_client_tools`` just like every other agent tool), and
``subagents`` nests recursively one level deep — the same nesting the core
:func:`tai_agents.deep_agent.factory.build_deep_agent` validator enforces.

``resolve_subagent_specs`` turns these JSON specs into core ``ResolvedSubAgentSpec``
objects with their tool names resolved to ``StructuredTool`` instances, ready for
:meth:`tai_agents.deep_agent.agent.DeepAgent.run` to build and run the deep agent.
``response_format`` is a JSON Schema dict (the deep-agent backend accepts a raw
schema as well as a pydantic model), so it crosses the tool boundary unchanged.
"""
...
from tai_agents._internal.reject import reject_untitled_response_format
...
class DeepSubAgentSpec(BaseModel):
    ...
    The JSON-expressible mirror of :class:`ResolvedSubAgentSpec`: ``tools`` are client
    tool names resolved at call time, and ``subagents`` nests one level deep.
    ``llm_provider`` / ``llm_kwargs`` are optional — when omitted the subagent
    inherits the main agent's model — and ``tools`` defaults to inheriting the
    parent's tools when empty.

    ``extra="forbid"`` rejects any key this shape does not carry loudly at
    validation rather than silently dropping it, matching the strictness of the
    enclosing ``DeepAgentInput``. In particular a per-sub ``strategy`` — which the
    deepagents sub-agent path cannot honor — is rejected here rather than vanishing.
...
async def _resolve_subagent_spec(spec: DeepSubAgentSpec) -> ResolvedSubAgentSpec:
    ...
    tools = await tai_app.tools.get_client_tools(spec.tools) if spec.tools else []
