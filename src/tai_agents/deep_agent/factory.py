...
This is the reusable core of the platform: it takes already-resolved
infrastructure (LLM, store, checkpointer) and the declarative inputs (tools,
subagents, skills, system prompt) and assembles a ``deepagents`` agent wired to
the composite backend.

Context overflow is intentionally left to deepagents' built-in summarization
(part of its base middleware stack for the main agent and every subagent). We
do not inject a project-wide context-overflow stack here: deepagents always
summarizes, so adding one on top would double-compact. The plain
``create_agent`` tools agents (which have no built-in compaction) keep using the
project context-overflow middleware; deep agents rely on the harness.
"""
...
from tai_kit.llm.settings import llm_settings
...
def _validate(
    ...
    duplicate names or off-root skill paths fail silently inside the harness
    (ambiguous routing, or a skill that resolves to the scratch backend and
    loads nothing). Mirrors the duplicate-name guard ``resolve_tools`` enforces
    over an agent's tools.
    """
    ...
    # A subagent named like a built-in tool (e.g. "task") is confusing/ambiguous.
    # (Naming a subagent "general-purpose" deliberately overrides deepagents'
    # default general-purpose subagent, so it is intentionally NOT rejected.)
    ...
    # Only the reference ``skills`` list is path-checked: those are source paths
    # the caller must root under SKILLS_ROOT. Inline skills are NAMES (mapped to
    # SKILLS_ROOT<name>/ internally), so they are validated separately, not here.
...
async def _resolve_subagent(
    ...
    *,
    ...
    Only the keys the caller actually set are emitted, so deepagents applies its
    own inheritance for the rest (parent model, parent tools, inherited
    ``interrupt_on``).

    ``llm`` / ``tools`` / ``store`` / ``backend`` are the main agent's resolved
    pieces; they are required only when ``spec.subagents`` is non-empty — nested
    subagents are compiled into their own deep agents (full default middleware
    stack, skills included) and attached to this subagent through a
    :class:`SubAgentMiddleware`, since deepagents' inheritance covers one level
    only.
    """
    sub: SubAgent = {
        "name": spec.name,
...
async def _compile_nested_subagent(
    ...
    *,
    ...
    The child is built as its own deep agent (so it gets the full default
    middleware stack — todos, filesystem, summarization — plus its skills) and
    handed to the parent's ``SubAgentMiddleware`` as a runnable. Model and
    tools follow the same inheritance the spec documents: the parent's, unless
    the child sets its own.
    """
    model = parent_model
...
async def build_deep_agent(
    *,
    ...
    (:class:`~tai_agents.deep_agent.agent.DeepAgent` resolves them from the kit
    registries before calling this). ``subagents`` are declarative specs resolved to deepagents
    ``SubAgent`` dicts; a spec may carry one level of nested ``subagents``, each
    compiled into its own deep agent reachable through the parent subagent's
    ``task`` tool. ``skills`` are source paths under
    :data:`tai_agents.deep_agent.backend.SKILLS_ROOT`.

    ``inline_skills`` are skills supplied directly in the call (each a name +
    ``SKILL.md`` body). They are collected from the main agent and every
    subagent into one mount served by the skills backend, and each agent's own
    inline skills are auto-loaded as ``SKILLS_ROOT<name>/`` sources — so
    supplying an inline skill loads it without also listing it in ``skills``.

    ``response_format`` (a pydantic model or langchain response strategy) makes
    the agent return a validated structured object in ``state['structured_response']``
    instead of free-form text; ``None`` keeps the free-form text behavior.
    """
