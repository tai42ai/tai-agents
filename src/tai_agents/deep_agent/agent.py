...
* :meth:`DeepAgent.run` — the JSON tool-face. Renders the messages, resolves
  ...
  on an interrupt raises :class:`~tai_contract.agent.base.AgentInterruptedError`
...
from tai_contract.agent.events import InterruptFinal, StreamEvent, StructuredFinal
...
from tai_kit.llm.runtime import build_agent_input
...
async def _neutral_to_internal(spec: NeutralSubAgentSpec) -> ResolvedSubAgentSpec:
    ...
    tools = await resolve_tools(tai_app.tools, list(spec.tool_names), list(spec.tools), list(spec.presets))
...
@tai_app.agents.agent("deep_agent")
class DeepAgent(Agent):
    ...
    async def run(
        ...
        **_: Any,
        ...
        :class:`~tai_contract.agent.base.AgentInterruptedError` rather than
        ...
        client_tools = await tai_app.tools.get_client_tools(list(tool_names)) if tool_names else []
    ...
    def _run_config(
        ...
        :func:`~tai_agents._internal.config_util.build_run_config` helper. With no
