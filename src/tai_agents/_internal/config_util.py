...
The active monitoring backend is reached through the ``tai_app`` handle
(``tai_app.monitoring.active``); the trace-context type comes from the
...
def init_langgraph_config(config: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    callbacks = tai_app.monitoring.active.writer.get_monitoring_callbacks(ctx)
