...
Each entry in ``voters`` is a ``VoterSpec`` (provider + optional model + LLM
kwargs) and spawns one voter; a list lets the same provider appear more than
once and lets each voter pin its own model. Every voter answers the same
rendered ``voter_message`` in parallel. The judge then receives the rendered
``judge_message`` followed by one line per voter (provider, model, verdict) and
produces the final decision by majority vote, breaking ties with its own
reasoning.

Only the JUDGE streams. The voters run silently in the prelude and their verdicts
are collected before the judge starts: their parallel runs do not map onto one
coherent event stream, so streaming the decisive stage — the judge that weighs
them — is deliberate, not a partial view. ``astream`` therefore emits the judge's
per-step events and ends with a terminal ``StructuredFinal`` carrying the full
``VotingOutput`` (judge verdict + every voter verdict); ``run`` drains that same
stream to the ``VotingOutput``.
"""
...
from tai_contract.agent.events import MessageFinal, RunUsage, StreamEvent, StructuredFinal
from tai_contract.app import tai_app
from tai_kit.llm.settings import llm_provider_settings

from tai_agents._internal.base_tool_agent import ainvoke_tools_agent
from tai_agents._internal.reject import reject_unhonored
from tai_agents._internal.render import render_message
from tai_agents._internal.stream_events import astream_tools_agent_events
from tai_agents.settings import agents_limits_settings
from tai_agents.voting_agent.model import VoteInfo, VoterSpec, VotingOutput
from tai_agents.voting_agent.prompt import JUDGE_SYSTEM_MESSAGE, VOTER_SYSTEM_MESSAGE
...
async def _run_voters(
    ...
    — the shared prelude for the judge stream. ``judge_llm_provider`` /
    ``judge_llm_kwargs`` are returned already defaulted (never ``None``).
    ``user_messages`` is the rendered judge message followed by one line per
    voter (provider, model, verdict). When ``voters`` is empty, a single voter
    runs on the judge's provider and kwargs.

    Each ``VoteInfo.model`` records the model the voter actually ran on (as
    reported by its run usage) when known, else the model its spec pinned, else
    ``None`` — never a misleading placeholder.
    """
...
# set). Voting drives its judge and voters through role-named params
# (``judge_*``/``voter_*``) plus ``checkpoint_provider``, so a generic contract
# param carries no meaning here and is rejected loudly rather than silently dropped.
# ``response_format`` is a conditional key: ``VotingOutput`` (the type this agent
# always produces) is accepted and removed before the guard runs, any other type is
# an offender named alongside the rest. ``checkpoint_provider`` is honored, so it
# never appears below.
...
# to ``None`` and is set when it is not ``None`` (so ``thread_id=None`` passes but a
# falsy-but-meaningful ``strategy=""`` still raises).
...
def _reject_unhonored(face: str, response_format: Any, extra_kwargs: dict[str, Any]) -> None:
    ...
    ``response_format`` folds into the same shared :func:`reject_unhonored` guard as
    every other unhonored parameter: ``VotingOutput`` (the type this agent always
    produces) — or an unset ``None`` — is accepted and dropped before the guard runs,
    any other type is an offender named alongside the rest. So a caller passing a
    foreign ``response_format`` AND another unhonored parameter is told about both in
    a single run rather than one per run. ``face`` is the fully qualified
    ``voting_agent.run`` / ``voting_agent.astream`` label named in the message."""
...
@tai_app.agents.agent("voting_agent")
class VotingAgent(Agent):
    ...
    async def run(self, **kwargs: Any) -> VotingOutput:
        """Drain ``astream`` to its terminal ``VotingOutput``. A caller-passed
        ``response_format`` other than ``VotingOutput``, or any ABC ``Agent.run``
        parameter the runtime has no seat for, is rejected before draining rather
        than silently dropped. The internal drain always forces the ``VotingOutput``
        structured terminal — a missing structured output raises rather than
    ...
    async def astream(
        ...
        **kwargs: Any,
        ...
        """Resolve the judge/voter tool names to live client tools, run the voters
        silently, stream the judge's events, then yield a terminal
        ``StructuredFinal`` carrying the full ``VotingOutput``.

        A caller-passed ``response_format`` other than ``VotingOutput``, or any ABC
        ``Agent.run`` parameter the runtime has no seat for, is rejected loudly here
        — in parity with :meth:`run` — rather than silently dropped."""
        _reject_unhonored("voting_agent.astream", response_format, kwargs)
        resolved_judge_tools: list[StructuredTool] = (
            await tai_app.tools.get_client_tools(judge_tools) if judge_tools else []
        )
        resolved_voter_tools: list[StructuredTool] = (
            await tai_app.tools.get_client_tools(voter_tools) if voter_tools else []
        )
