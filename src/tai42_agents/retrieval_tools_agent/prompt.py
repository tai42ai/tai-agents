...
The agent does not receive its tools up front. Instead it must ask for them by
describing what it needs; a semantic search over the embedded tool descriptions
returns the matching tools, which are then bound for the next step. This prompt
teaches that retrieve-then-use protocol and pins the terminal output contract:
every step ends with a single JSON object carrying a ``status`` field.

The ``status`` value is load-bearing — the graph's routing node parses this exact
JSON to decide whether to loop again (``continue``) or terminate (``success`` /
``error``). The prompt and :mod:`tai42_agents.retrieval_tools_agent.graph` must stay
in lockstep: the prompt emits the status JSON that the routing node consumes.
"""
