...
Importing this package registers ``DeepAgent`` through ``tai42_app`` (the
``@tai42_app.agents.agent("deep_agent")`` decorator runs on import of
:mod:`tai42_agents.deep_agent.agent`). The host loads the package via the
manifest's ``agents[].module`` entry (``tai42_agents.deep_agent``).
