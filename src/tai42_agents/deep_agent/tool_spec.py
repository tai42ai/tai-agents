...
The core :class:`tai42_agents.deep_agent.spec.ResolvedSubAgentSpec` carries a
...
``tai42_app.tools.get_client_tools`` just like every other agent tool), and
``subagents`` nests recursively one level deep — the same nesting the core
:func:`tai42_agents.deep_agent.factory.build_deep_agent` validator enforces.
...
:meth:`tai42_agents.deep_agent.agent.DeepAgent.run` to build and run the deep agent.
