...
``VOTER_SYSTEM_MESSAGE`` drives each voter LLM to answer a prompt as accurately
as it can, knowing its answer is checked against other models.
``JUDGE_SYSTEM_MESSAGE`` drives the judge LLM to take every voter's verdict and
produce the final decision by majority vote, breaking ties with its own
reasoning.

The prompts are assembled from adjacent string literals only so no physical
source line exceeds the line-length limit; each fragment carries its own spacing
and newlines, so the rendered prompt text is exactly the two multi-line blocks.
"""
