...
model-ready content part through the ``tai42_app.storage.resource_manager`` handle
(``normalize_media``), so a stored image resolves to a base64 data-URI while a
public URL passes through untouched; model access goes through tai42-kit's llm
